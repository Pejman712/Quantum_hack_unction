"""
Validate the MPS peak-finding pipeline against known-correct easy bitstrings.

Reads the success rows from results/easy_bitstrings.csv as ground truth,
runs matrix_product_operators on each challenge QASM, and asserts the
predicted bitstring matches.

Run with:
    uv run pytest tests/test_easy_validation.py -v
    uv run pytest tests/test_easy_validation.py -v -k "challenge_8"   # one circuit
"""
...
def _ground_truth() -> dict[str, str]:
    ...
    latest: dict[str, str] = {}
    for rec in records:
        if rec.difficulty == "easy" and rec.status == "success":
...
# ── parametrize one test case per easy challenge ─────────────────────────────
...
def test_mps_matches_csv_bitstring(challenge: str, expected: str, bond_dim: int, device: str) -> None:
    """MPS prediction for each easy challenge must match the CSV success entry."""
    qasm_path = find_challenge(challenge, data_dir=DATA_DIR / "easy")
