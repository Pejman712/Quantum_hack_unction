...
def amplitude(qc: QuantumCircuit, bitstring: str, *, optimize: str = "auto-hq") -> complex:
...
def probability(qc: QuantumCircuit, bitstring: str, *, optimize: str = "auto-hq") -> float:
    ...
    """
    amp = amplitude(qc, bitstring, optimize=optimize)
    return float((amp.conjugate() * amp).real)


def single_qubit_marginals_z(qc: QuantumCircuit, *, optimize: str = "auto-hq") -> list[float]:
